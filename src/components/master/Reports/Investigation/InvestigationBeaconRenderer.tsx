...
const InvestigationBeaconRenderer: React.FC<BeaconRendererProps> = ({
  id,
  x,
  y,
  area,
  ...
  const isVisitor = visitorsData.some((v) => v.id === id);
  const isMember = membersData.some((m) => m.id === id);
  const isSecurity = securityData.some((s) => s.id === id);
...
export default InvestigationBeaconRenderer;
