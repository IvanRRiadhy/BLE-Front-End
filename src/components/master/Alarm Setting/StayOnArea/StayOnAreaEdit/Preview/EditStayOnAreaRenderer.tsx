...
const EditStayOnAreaRenderer: React.FC<{
  width: number;
  height: number;
  ...
  activeStayOnArea?: StayOnAreaAlarmType;
  otherStayOnAreas?: StayOnAreaAlarmType[];
  ...
  const [activeStayOnAreaArea, setActiveStayOnAreaArea] = useState(activeStayOnArea?.name || '');
  const [areaDragging, setAreaDragging] = useState(false);
  const [dragOffset, setDragOffset] = useState({ dx: 0, dy: 0 });
  const [isColliding, setIsColliding] = useState(false);
  const drawingStayOnArea = useSelector(
  ...
  useEffect(() => {
    setActiveStayOnAreaArea(activeStayOnArea?.name || '');
    // console.log('Active StayOnArea changed:', activeStayOnArea?.areaShape);
  }, [activeStayOnArea]);
  ...
  const checkCollisionWithOffset = (areaName: string, dx: number, dy: number): boolean => {
    if (!activeStayOnArea) return false;

    const proposedArea = {
      nodes: activeStayOnArea.nodes
        ? activeStayOnArea.nodes.map((node) => ({
            ...node,
            x: node.x + dx * scale,
            y: node.y + dy * scale,
            x_px: node.x_px + dx,
    ...
    if (otherStayOnAreas === undefined) return false;
    return otherStayOnAreas.some((otherArea) => {
  ...
  const checkCornerDragCollision = (cornerIndex: number, newX: number, newY: number): boolean => {
    ...
    const proposedArea = {
      nodes: activeStayOnArea.nodes
        ? activeStayOnArea.nodes.map((node, index) =>
  ...
  const handleCanvasClick = () => {
    if (!drawingStayOnArea) return; // Only allow drawing if the drawing mode is active
    ...
    if (!activeStayOnArea) return;
    setDrawingNodes((prevNodes) => {
      const updatedNodes = [...prevNodes, newNode];
      let collision = false;
      if (otherStayOnAreas === undefined) {
        collision = false;
      } else {
        collision = otherStayOnAreas.some((area) => {
      ...
      if (collision) {
        ...
        setDrawingNodes([]);
        dispatch(DrawStayOnArea('')); // Reset the drawing mode
      ...
      if (updatedNodes.length === 3) {
        ...
        setDrawingNodes([]);
        dispatch(DrawStayOnArea('')); // Exit drawing mode after completing the polygon
      }
      return updatedNodes;
    });
  };
  ...
  useEffect(() => {
    if (drawingStayOnArea === undefined) return;
    if (drawingStayOnArea !== '') {
      console.log('Drawing mode active for StayOnArea ID:', drawingStayOnArea);
      setCursor('crosshair');
    } else {
      setCursor('default');
    }
  ...
  const handleDragStart = (e: string) => {
    if (drawingStayOnArea) return; // Prevent dragging while drawing
    setIsDragging(e);
  ...
  const handleCornerDragStart = (areaName: string, cornerIndex: number) => {
    ...
    const corner = activeStayOnArea.nodes && activeStayOnArea.nodes[cornerIndex];
    if (!corner) return;
    setCornerDragData({
      areaName,
      cornerIndex,
  ...
  const handleDragCorner = (areaName: string, cornerIndex: number, x: number, y: number) => {
    const newNodes = [...(activeStayOnArea?.nodes || [])];
    ...
    const updatedStayOnArea = {
      ...activeStayOnArea,
      nodes: newNodes,
      areaShape: JSON.stringify(newNodes),
    };
    // console.log('updatedStayOnArea', updatedStayOnArea);
    if (updatedStayOnArea) {
      dispatch(UpdateSelectedStayOnAreaAlarm(updatedStayOnArea));
    }
  };
  ...
  const handleCornerDragEnd = (areaName: string, cornerIndex: number, x: number, y: number) => {
    ...
    const proposedNodes = activeStayOnArea.nodes ? [...activeStayOnArea.nodes] : [];
    proposedNodes[cornerIndex] = { ...proposedNodes[cornerIndex], x: dPxX, y: dPxY };

    // Check for both collisions and self-intersections
    if (otherStayOnAreas === undefined) return false;
    const hasCollision = otherStayOnAreas.some((otherArea) => {
      if (!otherArea.nodes) return false; // Add this check
    ...
    if (hasCollision) {
      ...
      if (cornerDragData) {
        ...
        if (!activeStayOnArea.nodes) return false;
  ...
  const handleDragArea = (areaName: string, dx: number, dy: number) => {
    ...
    const updatedStayOnArea = {
      ...activeStayOnArea,
      nodes: activeStayOnArea.nodes?.map((node) => ({
        ...node,
        x: node.x + dx * scale,
        y: node.y + dy * scale,
        x_px: node.x_px + dx,
  ...
  const handleRightClick = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (!drawingStayOnArea) return;

    // Cancel drawing
    setDrawingNodes([]); // Clear current drawing nodes
    dispatch(DrawStayOnArea('')); // Reset drawing mode
    ...
    <>
      ...
      >
        <Layer>
          ...
          {activeStayOnArea && (
            <React.Fragment key={activeStayOnArea.id}>
              <Line
                points={activeStayOnArea.nodes ? setPointsFromNodes(activeStayOnArea.nodes) : []}
                stroke={isColliding ? 'red' : darken(activeStayOnArea.color, 0.5)}
                strokeWidth={5}
                lineJoin="round"
                lineCap="round"
                closed
                fill={activeStayOnArea.color}
                opacity={0.7}
                draggable
                onMouseEnter={() => {
                  if (!drawingStayOnArea) setCursor('move');
                }}
                onMouseLeave={() => {
                  if (!drawingStayOnArea) setCursor('grab');
                }}
                onMouseDown={(e) => {
                  if (!drawingStayOnArea) {
                    setIsDragging(activeStayOnArea.name);
                    handleDragStart(activeStayOnArea.name);
                ...
                onDragEnd={(e) => {
                  handleDragEnd(activeStayOnArea.name);
                  e.target.x(0);
                  e.target.y(0);
                  setIsDragging('');
                }}
              ...
              {!areaDragging &&
                ...
                activeStayOnArea.nodes.map((node, index) => (
                  <Circle
                    ...
                    onDragStart={() => {
                      setIsDragging(activeStayOnArea.name);
                      handleCornerDragStart(activeStayOnArea.name, index);
                    }}
                    onDragMove={(e) => {
                      handleDragCorner(
                        activeStayOnArea.name,
                    ...
                    onDragEnd={(e) => {
                      ...
                      handleDragEnd(activeStayOnArea.name); // Pass the activeStayOnArea name
                    }}
                    onContextMenu={(e) => {
                      e.evt.preventDefault(); // Prevent the default context menu from appearing
                      handleDeleteCorner(activeStayOnArea.name, index); // Call the function to delete the corner
                    }}
                  />
                ))}
            </React.Fragment>
...
export default EditStayOnAreaRenderer;
