...
import { ExportCard } from 'src/store/apps/crud/card';
...
const CardExport = () => {
  ...
  const handleExport = async (type: 'pdf' | 'excel') => {
    ...
    try {
      ...
      await Promise.all([dispatch(ExportCard(type)).unwrap(), delay(1000)]);
      toast.success(`Exported Card as ${type.toUpperCase()}`);
    } catch (error) {
