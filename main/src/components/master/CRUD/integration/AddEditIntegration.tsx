...
import { defaultIntegrationForm } from 'src/store/apps/defaultForm';
...
const AddEditIntegration = ({ type, integration }: FormType) => {
  ...
  const handleClickOpen = () => {
    if (type === 'edit' && integration) {
      ...
      setFormData({ ...defaultIntegrationForm, ...integration });
    } else {
      setFormData({ ...defaultIntegrationForm });
    }
    setTimeout(() => {
      setLoading(false);
      setOpen(true);
  ...
  const handleSave = async () => {
    try {
      let result;
      if (type === 'edit') {
        result = await dispatch(editIntegration(formData)); // Dispatch update
      }
      if (type === 'add') {
        result = await dispatch(addIntegration(formData));
      }
            ...
            <Grid container spacing={5} mb={3}>
              ...
              <Grid size={{ lg: 6, md: 12, sm: 12 }} direction="column">
                ...
                <CustomTextField
                  ...
                  variant="outlined"
                />
                <CustomFormLabel htmlFor="api-key-field">
