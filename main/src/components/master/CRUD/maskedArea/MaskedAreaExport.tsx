...
import { ExportMaskedArea } from 'src/store/apps/crud/maskedArea';
import toast from 'react-hot-toast';

const MaskedAreaExport = () => {
  ...
  const handleExport = async (type: 'pdf' | 'excel') => {
    ...
    try {
      ...
      await Promise.all([dispatch(ExportMaskedArea(type)).unwrap(), delay(1000)]);
      toast.success(`Exported Masked Area as ${type.toUpperCase()}`);
    } catch (error) {
      console.error('Export Error:', error);
      toast.error('Failed to export Masked Area');
    } finally {
...
export default MaskedAreaExport;
