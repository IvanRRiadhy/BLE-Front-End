...
import EditPatrolAreaRenderer from './EditPatrolAreaRenderer';
...
const EditPatrolAreaFloorView: React.FC<{
  zoomable: boolean;
  preview?: boolean;
  ...
  const activePatrolArea = useSelector(
  ...
  useEffect(() => {
    const filteredPatrolArea = unsavedPatrolAreas.filter(
      (patrolArea: PatrolAreaType) => patrolArea.floorplanId === activeFloorPlan?.id,
    );
    setFilteredUnsavedPatrolArea(filteredPatrolArea);
  }, [unsavedPatrolAreas, activeFloorPlan]);
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isDragging || drawingPatrolArea) {
        return;
      }
    ...
    [zoomable, isDragging, drawingPatrolArea, stagePos, cursor],
  );
  ...
  return (
    ...
    >
      ...
      {editingPatrolArea && !drawingPatrolArea && zoomable && (
        <Box
          sx={{
            position: 'fixed',
            top: 160,
      ...
      {/* Zoom Controls */}
      {isHovered && zoomable && !isDragging && !drawingPatrolArea && (
        <ZoomControls
          scale={stageScale}
...
export default EditPatrolAreaFloorView;
