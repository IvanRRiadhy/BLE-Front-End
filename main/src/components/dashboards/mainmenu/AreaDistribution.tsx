...
import { DashboardAreaChartType } from 'src/store/apps/dashboard/Dashboard';
...
interface AreaDistributionProps {
  data: DashboardAreaChartType[];
  outerRadius?: number;
  label?: string;
}

const AreaDistribution: React.FC<AreaDistributionProps> = ({
  data = [],
  outerRadius = 80,
  label = 'Area Usage',
}) => {
    const chartData = data.map(d => ({
    name: d.areaName ?? 'Unknown Area',
    value: d.totalRecords ?? 0,
  }));
  ...
  const areaColors = generateColors(chartData.length);
